...
# If the `-s or --setup` flag is passed, set setup to true.
if has_param '-s' "$@" || has_param '--setup' "$@"
then
    >&2 echo -e "${BLUE}Setup requested${NC}"
    setup=true
fi
...
if [ "${setup}" = true ]
then
    # Ensure the MIX_SETUP envirnoment variable is set to true.
...
then
    ...
    function check_status() {
        ...
        local status_code=$(curl --write-out %{http_code} --silent --output /dev/null localhost:${PORT}/healthcheck)
        ...
        then
            ...
            # Cursor visible again.
            tput cnorm
            >&2 echo -e "${GREEN}The server is Up at localhost:${PORT}${NC}"
    ...
    >&2 echo -e "${YELLOW}* Checking if the server is Up at localhost:${PORT}${NC} ..."
    progress &
